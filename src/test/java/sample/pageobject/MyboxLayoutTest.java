...
public class MyboxLayoutTest{
	...
	public void 登録ファイル確認_マイボックス表示_画像ファイル() throws Exception{
	    // ログイン情報
	    String txtId = "testselenium147258369@gmail.com";
	    String txtPw = "abcd1234";
	    // 使用するファイルパス
	    ...
	    for (int second = 0;; second++) {
	    	...
	    	try { if (driver.findElement(By.cssSelector("div.LinkArea")).isDisplayed()) break; } catch (Exception e) {}
	    	Thread.sleep(1000);
	    }
	    ...
	    for (int second = 0;; second++) {
	    	if (second >= 60) fail("timeout");
	    	try { if (driver.findElement(By.cssSelector("td.modalContent")).isDisplayed()) break; } catch (Exception e) {}
	    	Thread.sleep(1000);
	    }
	    ...
	    for (int second = 0;; second++) {
	    	if (second >= 60) fail("timeout");
	    	try { if (driver.findElement(By.xpath("//div[@id='modal']/table/tbody/tr[2]/td[2]")).isDisplayed()) break; } catch (Exception e) {}
	    	Thread.sleep(1000);
	    }
